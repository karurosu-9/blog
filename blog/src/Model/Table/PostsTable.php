...
class PostsTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        $validator
            ...
            ->notEmptyString('title', '入力必須項目です');

        $validator
            ->scalar('description')
            ->maxLength('description', 100, '100文字以下で入力してください')
            ->minLength('descripton', 3, '3文字以上で入力してください')
            ->requirePresence('description', 'create')
            ->notEmptyString('description', '入力必須項目です');

        $validator
            ->scalar('body')
            ->requirePresence('body', 'create')
            ->notEmptyString('body', '入力必須項目です');
        ...
        $validator
            ->multipleOptions('tags._ids', ['min' => 1],  '最低1つはタグを選択してください');
